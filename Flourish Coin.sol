...
contract FlourishCoin is Context, IERC20, Ownable {
    ...
    address public _presaleContract = address(0);
    ...
    function changePresale(address newP) external onlyOwner() {
        _presaleContract = newP;
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && from != uniswapV2Pair && from != _presaleContract && (_tOwned[from] >= _maxTxAmount.mul(5))) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            ...
            _transferStandard(sender, recipient, amount.sub(charityAmt));
            removeAllFee();
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require (taxFee <= 3, "Up to 3%");
        _taxFee = taxFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require (liquidityFee <= 7, "Up to 7%");
        _liquidityFee = liquidityFee;
    }

    function setCharityFeePercent(uint256 charityFee) external onlyOwner() {
        require (charityFee <= 2, "Up to 2%");
        _charityFee = charityFee;
    }
     
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require (maxTxAmount >= 1e11, "Bigger than 0.1T");
        _maxTxAmount = maxTxAmount * 10**9;
    }
    
    function setMarketingDivisor(uint256 divisor) external onlyOwner() {
        require (divisor <= 4, "Up to 4%");
